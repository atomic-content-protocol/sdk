...
### Security
- **server:** Rate limiting now keys on a stable client address taken from the platform's own header (`CLIENT_IP_HEADERS`, default `x-envoy-external-address,cf-connecting-ip,true-client-ip,x-real-ip`) instead of `req.ip`. With `trust proxy: N`, Express skips N hops from the right of `X-Forwarded-For`, so a chain of `client, edge1, edge2` resolved to an internal address that varied per request — every request received a fresh bucket and the hourly limit was never enforced in production. The daily spend cap was unaffected (it is process-global, not keyed by client).
