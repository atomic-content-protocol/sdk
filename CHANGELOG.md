...
- **deploy:** Railway build no longer runs a second `npm ci` inside Nixpacks (which hit `EBUSY` on the mounted `node_modules/.cache`); install is left to the Nixpacks install phase and the build command only builds.
