...
- **deploy:** Railway build fixed twice over: the build command no longer runs a second `npm ci` inside Nixpacks (which hit `EBUSY` on the mounted `node_modules/.cache`), and the Nixpacks install phase uses `npm ci --include=dev` because Railway sets npm's `production` config, which stripped TypeScript, turbo and `@types/*` and broke `tsc`. Verified by simulating both phases from a clean checkout with `NPM_CONFIG_PRODUCTION=true`.
