...
import { fetchPageForUrl, isExtractionTooThin } from "./utils/fetch-url.js";
...
export async function createACO(params: CreateACOParams): Promise<ACO> {
  ...
  if (hasUrl) {
    try {
      // fetchPageForUrl rather than fetchBodyForUrl so the page's own <title>
      // is available to the thin-body check. `params.title` is undefined for
      // the common createACO({ url }) call, which left that check dead.
      const page = await fetchPageForUrl(params.url as string);
      body = page.text;
      ...
      fetchStatus = isExtractionTooThin(body, params.title ?? page.title)
        ? {
            ok: false,
            permanent: true,
