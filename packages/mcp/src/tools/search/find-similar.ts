...
import { extractEntityNames, extractStrings, idOf, jaccard, titleOf, tokens } from "../../utils/frontmatter.js";
...
function acoOverlap(source: ACO, candidate: ACO): number {
  ...
  const entityScore = jaccard(
    extractEntityNames(source.frontmatter["key_entities"]),
    extractEntityNames(candidate.frontmatter["key_entities"])
  );
      ...
      if (ctx.hasEnrichment && typeof ctx.storage.findSimilar === "function") {
        try {
          ...
          if (provider.embed) {
            ...
            const hits = (await ctx.storage.findSimilar(vector, { limit: limit + 1, threshold })).filter(
              (r) => r.id !== id
            );
            if (hits.length > 0) {
              return {
      ...
      const results = all
        .map((candidate) => ({
          ...
          similarity_score: Number(
            (source ? acoOverlap(source, candidate) : textOverlap(queryText, candidate)).toFixed(4)
          ),
        }))
